...
enum Phase { BETWEEN_HOURS, ADMITTING, AWAITING_ASSIGNMENT, PROCESSING, AWAITING_CHOICE, CLOSED }
var phase: Phase = Phase.CLOSED
var hour_in_progress: bool:
	get:
		return phase in [Phase.ADMITTING, Phase.AWAITING_ASSIGNMENT, Phase.PROCESSING, Phase.AWAITING_CHOICE]
var night_serial: int = 0
var pending_client_queue: Array = []
var pending_table_assignments: Array = []
...
func advance_hour() -> Dictionary:
	...
	pending_client_queue = _generate_clients_for_hour(hour + 1)
	return _result(["[color=yellow]-- Hour %d (%s–%s) --[/color]" % [hour + 1, _format_time(hour), _format_time(hour + 1)]])

func process_next_client_entry() -> Dictionary:
	if phase != Phase.ADMITTING:
		return _result()
	while not pending_client_queue.is_empty():
		var client = pending_client_queue.pop_front()
		var slot = _first_empty_slot()
		if slot < 0 or rng.randf() >= GameConfig.CLIENT_ENTRY_CHANCE:
			continue
		var table = _make_table(slot, "Table %d" % slot, client["social_status"], client["race"], client["group_size"])
		table["is_unserved"] = true
		tables.append(table)
		var available = false
		for w in wenches:
			if w["current_state"] == "serving":
				available = true
		if available:
			pending_table_assignments = [table]
			phase = Phase.AWAITING_ASSIGNMENT
			return _result(["%s enters at %s. Select a staff portrait to assign." % [table["description"], table["label"]]])
		return _result(["%s enters at %s; all staff are unavailable, so the table is unserved." % [table["description"], table["label"]]])
	return _begin_table_processing()

func assign_wench_to_pending_table(wench_name: String, visit_id: int, expected_night: int) -> Dictionary:
	if phase != Phase.AWAITING_ASSIGNMENT or expected_night != night_serial or pending_table_assignments.is_empty():
		return _result()
	var table = pending_table_assignments[0]
	...
	for w in wenches:
		if w["name"] == wench_name and w["current_state"] == "serving":
			...
			_sync_assignments()
			phase = Phase.ADMITTING
			return _result(["[color=green]%s assigned to %s.[/color]" % [wench_name, table["label"]]])
	return _result()

func has_pending_table_assignment() -> bool:
	return phase == Phase.AWAITING_ASSIGNMENT and not pending_table_assignments.is_empty()

func get_next_pending_table_label() -> String:
	return pending_table_assignments[0]["label"] if has_pending_table_assignment() else ""

func _begin_table_processing() -> Dictionary:
...
func apply_choice(choice_id: String) -> Dictionary:
	...
	match selected["action"]:
		...
		"free_round":
			...
			logs.append("[b]You offer a free round at %s.[/b]" % table["label"])
...
func _append_stock_report(logs: Array) -> void:
	...
	for liquor_type in stock.keys():
		...
		logs.append("%s: %d pints" % [_colorize_liquor(display_name), stock[liquor_type]])
	logs.append("")
