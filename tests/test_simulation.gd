...
func _test_choices() -> void:
	...
	check(sim.gold == 3 and sim.stock == original_stock, "Comp refunds once without a second inventory deduction")
	sim.apply_choice(choice["id"])
	check(sim.gold == 3 and sim.phase == Simulation.Phase.PROCESSING, "Duplicate choice cannot apply twice")
