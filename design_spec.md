# Wench Manager — Reconciled playable night (v0.3)

This document describes the implemented milestone. The original long-term design
is preserved in `docs/recovered_design_archive.md`; its implementation claims are
...
5pm–1am. Start empty, advance an hour, assign each arriving group using portraits,
read events and resolve crises, adjust
...
- `HUD.gd`: resource labels, recovered table indicators and portrait availability.
- `TableIndicator.tscn`, `ui/TableIndicator.gd`: colored slot/assignment indicators.
- `WenchSelector.tscn`, `ui/WenchSelector.gd`, `portraits/`: recovered portrait selector and original art.
...
- `ADMITTING`: `process_next_client_entry()` drains arrivals until an assignment is needed.
- `AWAITING_ASSIGNMENT`: a staff portrait resolves exactly the pending visit for this night.
...
The fixed roster is Lysa, Brakka and Mimi. New visits pause for player assignment
through the recovered portrait selector. If all staff are unavailable, arrivals
remain unserved without creating an impossible prompt. After admissions finish,
uncovered visits receive an available staff member with the fewest active tables.
Existing valid assignments are preserved. The player can select a different available staff member through
...
The recovered branch's transaction behavior and 10–30% tip formula are retained.
Its free-round action uses additional inventory without refunding prior sales;
that replaces the earlier improvement branch's comp/refund interpretation.
Duplicate actions still cannot repeat the transaction. Regular service only
reports a stock shortfall when less than the requested quantity is available.
Resource names retain the recovered color-coded narration.
...
socializing/invitation mechanics, campaign persistence and AI narration
...
See `docs/reconciliation.md` for branch provenance, retained features, fixes, and
